...
export default function CompanyDetail() {
  ...
  const [ensureStep, setEnsureStep] = useState<string | null>(null);
  ...
  const ensureCompanyData = useCallback(async () => {
    if (!id || !company || ensureRunning || ensureRanRef.current) return;
    ...
    try {
      if (!hasSignals) {
        setEnsureStep("Searching for signals…");
        try {
      ...
      if (companyAny?.domain && meetings.length === 0) {
        setEnsureStep("Syncing meetings…");
        try {
      ...
      if (!hasContacts && isPartnerCategory) {
        setEnsureStep("Finding contacts…");
        try {
      ...
      if (!hasCompanyCards) {
        setEnsureStep("Analyzing company…");
        try {
          await supabase.functions.invoke("generate-cards", {
      ...
      if (companyAny?.domain) {
        try {
          const { data: freshMeetings } = await supabase
            ...
            .is("transcript_analysis", null);
          if (freshMeetings && freshMeetings.length > 0) {
            setEnsureStep("Analyzing transcripts…");
            for (const m of freshMeetings) {
              try {
      ...
      if (hasContacts && !hasContactProfiles) {
        setEnsureStep("Building contact profiles…");
        try {
      ...
      if (!hasScore) {
        ...
        try {
          ...
          console.warn("Scoring non-fatal:", e.message);
        }
      }
      ...
      console.error("ensureCompanyData failed:", e.message);
    } finally {
      setEnsureRunning(false);
      setEnsureStep(null);
    }
  }, [id, company, ensureRunning, signals.length, companyCards, contacts, meetings.length, companyAny?.domain, isPartnerCategory, queryClient, updateCompany]);

  useEffect(() => {
    if (company && !isLoading && !ensureRanRef.current) {
      const timer = setTimeout(() => ensureCompanyData(), 500);
      return () => clearTimeout(timer);
    }
  }, [company, isLoading, ensureCompanyData]);
  ...
  return (
    <div>
              ...
              {viewMode === "contact" && effectiveContactId && (
                <>
                  ...
                  <Button
                    ...
                    disabled={!!generatingContactId || ensureRunning}
                  >
        ...
        <div>
          {ensureRunning && (
            <div className="flex items-center gap-2.5 py-3 mb-4 text-caption text-foreground/40">

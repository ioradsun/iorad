...
export default function LoginPage() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    // If no error, auth state change will redirect — don't reset signingIn
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-background">
      <div className="w-full max-w-sm mx-auto text-center space-y-8 px-6">
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm text-muted-foreground">Use your @iorad.com Google account to continue</p>
        </div>

        <button
          onClick={handleGoogleSignIn}
          disabled={signingIn}
          className="w-full flex items-center justify-center gap-3 px-6 py-3 rounded-xl bg-foreground text-background font-medium text-sm hover:opacity-90 transition-opacity disabled:opacity-60"
        >
          {signingIn ? (
            <>

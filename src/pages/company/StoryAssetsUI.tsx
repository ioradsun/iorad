      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {loom?.title && <div className="text-xs"><span className="font-mono text-muted-foreground">Title:</span> <span className="text-foreground/90">{loom.title}</span></div>}
            {loom?.purpose && <div className="text-xs"><span className="font-mono text-muted-foreground">Purpose:</span> <span className="text-foreground/90">{loom.purpose}</span></div>}
            ...
            {loom?.when_to_send && <div className="text-xs"><span className="font-mono text-muted-foreground">When to send:</span> <span className="text-foreground/90">{loom.when_to_send}</span></div>}
            {loom?.intro_message && (
              <div className="space-y-1">
                ...
                <Button size="sm" variant="ghost" className="gap-1 text-xs h-7" onClick={() => copy(loom.intro_message)}>
            ...
            {loom?.loom_script && (
              <div className="space-y-1 border-t border-border/50 pt-3">
                ...
                <Button size="sm" variant="ghost" className="gap-1 text-xs h-7" onClick={() => copy(loom.loom_script)}>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {iorad?.title && <div className="text-xs"><span className="font-mono text-muted-foreground">Title:</span> <span className="text-foreground/90">{iorad.title}</span></div>}
            {iorad?.environment && <div className="text-xs"><span className="font-mono text-muted-foreground">Environment:</span> <span className="text-foreground/90">{iorad.environment}</span></div>}
            ...
            {iorad?.business_outcome && <div className="text-xs"><span className="font-mono text-muted-foreground">Business outcome:</span> <span className="text-foreground/90">{iorad.business_outcome}</span></div>}
            {iorad?.when_to_send && <div className="text-xs"><span className="font-mono text-muted-foreground">When to send:</span> <span className="text-foreground/90">{iorad.when_to_send}</span></div>}
            {iorad?.intro_message && (
              <div className="space-y-1">
                <div className="text-[10px] font-mono text-muted-foreground">Intro Message</div>
                <div className="text-xs text-foreground/80 bg-secondary/30 rounded p-2">{iorad.intro_message}</div>
                <Button size="sm" variant="ghost" className="gap-1 text-xs h-7" onClick={() => copy(iorad.intro_message)}>
                  <Copy className="w-3 h-3" /> Copy Intro
                </Button>
              </div>
            )}
            {iorad?.embed_context && <div className="text-xs"><span className="font-mono text-muted-foreground">Embed context:</span> <span className="text-foreground/90">{iorad.embed_context}</span></div>}
